...
> ⚠️ 本节原为 Day 5 快照（当时 `js/` 还是空的）。**M7 收尾时已按实际重写**，就是下面这棵树；规划稿留在 4.2 供对照。
...
├── index.html                  ← 仓库首页占位页（Day 2，**不进发布**）
├── .github/workflows/pages.yml ← M7 发布流水线（见 11.2）
├── tools/
│   └── postprocess-art.py      ← 出图后处理（与站点同源，不进部署目录）
├── checkin/                    ← 每日打卡流水（另一条线，与本项目无关）
└── yizhi/                      ← ✅ 代码真源，也是**发布根目录**
    ├── index.html              ← 唯一页面，五个视图都在里面
    ├── css/
    │   ├── tokens.css          ← 令牌层：三色、字体、纸纹、动效
    │   ├── base.css            ← 页面层：题签、鱼尾、版框、书尾
    │   ├── views.css           ← 视图层：网格/谱系树/时间轴/六类分组/竖轴
    │   ├── detail.css          ← 详情面板 + 收藏列表
    │   └── voyage.css          ← 转场层（云中仙槎）
    ├── js/                     ← ✅ 15 个模块，见 4.2
    ├── data/                   ← 五部各一份 JSON，共 96 条
    │   ├── yishou.json         ← 异兽部 12 条
    │   ├── jingjie.json        ← 境界部 14 条（9 网文 + 5 真丹道）
    │   ├── shenxian.json       ← 神仙部 30 条
    │   ├── shenhua.json        ← 神话部 20 条
    │   └── yaoguai.json        ← 妖怪部 20 条
    ├── assets/                 ← 公开图 12 张（webp）+ SOURCES.md 来源台账
    ├── assets-private/         ← 私有图（不进 git，仅本机；当前为空）
    ├── svg/.gitkeep            ← 自绘矢量装饰目录
    ├── 预览/                    ← 出图样张与样式快照（**不进发布**）
...
> ✅ **M7 已全部落地**。实际与规划只差三处命名，以实际为准：
> ① `views/favlist.js` → **`views/fav.js`**；② 多出内部公共件 **`views/portrait.js`**（出图与图源判定，网格与详情共用）；
> ③ 样式按视图拆成 `views.css` / `detail.css` / `voyage.css` 三份，而不是只留一份 `views.css`。
> 另外**新增 `views/result.js`**：M6 起检索引擎改成全站，结果区不再复用网格，需要独立的「列表」形态（见 6.2）。
...
**部专属的顶层字段（M5/M6 补）**：`{schema, 部, 更新, 条目}` 是四部共用的最小包装；两部因为视图本身需要「分类/分期的说明」，各多一个顶层键。

| 部 | 顶层键 | 结构 | 为什么必须放在文件里 |
|---|---|---|---|
| 妖怪 | `形类释义` | `{妖,魔,鬼,怪,精,灵}` → 一句话界定 | 六类的分界是**内容判断**，不是样式。摆在每条目里会重复 20 遍，摆进 JS 就变成「改分类要去改代码」 |
| 妖怪 | `年表` | `[{期, 说明, 出处}]`，四期（先秦/汉魏六朝/唐/明清） | 分期是**史实**，要给得出处。视图只负责画，不负责知道分几期 |
| 境界 | `序列` | `['网文','真丹道']` | 一套数据支撑两套体系，视图要先知道「有哪几套」才能画开关 |

> 取舍口径：**凡是「改了要不要重新考据」的东西放数据，凡是「改了只影响样子」的东西放视图。** 分几类、分几期属于前者。
...
**部专属的条目字段（M5/M6 补）**：五部共用上表，但四个部各多几个字段 —— 视图的结构（树有几级、轴有几阶）必须由数据决定，不能写死在 JS 里。

| 部 | 字段 | 类型 | 必填 | 取值 / 用途 |
|---|---|---|---|---|
| 神仙 | `谱系` | string | ✅ | `三清` / `四御` / `职能部` —— 谱系树的**第一级** |
| 神仙 | `司` | string | 职能部填 | `雷` / `斗` / `兵` / `冥` / `山水` —— 树的**第二级**（三清、四御无此字段） |
| 神仙 | `座次` | number | ✅ | 同一层内排序 |
| 神话 | `类型` | string | ✅ | `创世` / `始祖` / `洪水` / `战争` / `发明` / `斗争` —— 时间轴的**分段** |
| 神话 | `序` | number | ✅ | **段内**顺序。段与段之间不排绝对年代（争不出结论），所以序号只在段内有意义 |
| 妖怪 | `形类` | string | ✅ | `妖` / `魔` / `鬼` / `怪` / `精` / `灵` —— 六类分组的依据 |
| 妖怪 | `座次` | number | ✅ | 组内排序 |
| 境界 | `序列` | string | ✅ | `网文` / `真丹道` —— 决定它出现在哪一套轴里 |
| 境界 | `阶号` | number | ✅ | 轴上第几阶；网文 1–9，真丹道 1–5 |
| 境界 | `原文性质` | string | ✅ | 标明这条的「原文」是**古籍真文**还是**当代通行设定**。网文九阶没有古籍出处，必须自己说清楚，**不许假装是古书** |

⚠️ **境界部（E 部）不写 `吉凶`**：`吉凶` 问的是「这兽对人好不好」，境界是修行阶段，没这个维度。`data.js` 对 E 部不做该校验。
...
renderResults(容器, 条目[])          // M6 全站检索结果列表（带部角标），与网格分开
```
...
> **M5/M6 补记（09-22）**：三个决定，都不是随手加的。
> ① **多出 `views/result.js`**。原计划「检索结果复用异兽网格」，M6 改成全站检索后不成立了：结果会跨部混排，卡片上必须挂**部角标**，而且进结果区时部内工具条（吉凶筛选、序列开关）要收起 —— 网格视图不认识「部角标」这个概念，硬塞会把 `renderGrid` 的契约撑破。**宁可多一个 60 行的视图，也不让一个视图承担两种语义。**
> ② **五部渲染由一张「部形态表」装配**（在 `main.js`）。表里每部记四项：视图函数、`视图台` 挂的 class（`grid` 带乌丝栏底 / `stage` 素底）、工具条装什么（`吉凶` / `序列` / 无）、给视图的额外实参。**新增一个部 = 表里加一行**，不改 `切部()` 的流程。
> ③ **检索引擎改为「懒建一次、全站共用」**。`loadAll()` 从「只在需要全量时调」变成「首次检索时调一次」，之后 `全站索引` 一直复用；并发调用共享同一个进行中的 Promise，避免连打字连着建五份索引。**代价**：首次搜索要等五个 JSON 读完，这段等待期不假装能搜 —— 明写「数据还在读，稍等一下再搜」（PRD 9.2：不给死胡同）。

**装配层的两个新增出口 `main.js`**（不在视图契约里，但跨模块调用要认）：

```js
确保条目(id)   →  Promise<条目|null>   // 当前包没有就按 id 前缀去载那一部；收藏与检索共用
部ofId(id)    →  string|null        // 由 id 前缀反查部名（'yaoguai-001' → '妖怪'）
```

> 为什么要 `确保条目`：M6 起收藏可以**跨部**。刷新后打开收藏夹，异兽部那份包不包含「白蛇」 —— 按 id 前缀补载那一部，它才不会被误判成「此条已下架」（§12.4 的占位是留给**真被删掉**的条目的）。
...
用户搜「妲己」→ 点结果 → 收藏（**M6 起是全站检索**，所以第一步不是 `loadBu` 而是先把五部备齐）：

```
main.js
  → 建全站索引()                    // 首次检索时：Promise.allSettled(五部取包) → buildIndex(96 条)，只建一次
  → search.query(索引, '妲己')       // 命中（靠 别名 里的「妲己」）→ 九尾狐（异兽部）
  → views.renderResults(视图台, ...)  // 结果卡片带「异兽」部角标；部内工具条收起
  → transit.play() → views.renderDetail(条目)
  → store.toggle(id)                // 写 localStorage
  → 同步收藏()                       // 详情按钮 + 入口计数 + 收藏面板列表，四处一起刷
```

> 对比 M4 之前：那时只搜异兽部，`loadBu('异兽')` 就够。改成全站的**硬理由**在 PRD §8.1 —— 「输入『九尾狐』结果不该只有一条」，只搜一部永远办不到。
...
| 发布目录 | `yizhi/`（**不是**仓库根目录） |
| 站点根 | = `yizhi/` 的内容 → 线上地址 **`https://simple-superhero-678.github.io/vibe-coding-100/`**（不带 `/yizhi/` 前缀） |
| 发布方式 | **GitHub Actions + artifact 直传**（`.github/workflows/pages.yml`，M7 已落） |
| 自定义域名 | 无 |
| 状态 | ✅ **M7 已完成**（workflow 已入仓；GitHub 网页上的 `Settings → Pages → Source = GitHub Actions` 需人工点一次） |
...
| 做法 | 怎么做 | 代价 |
|---|---|---|
| **A · GitHub Actions（✅ M7 选定）** | workflow 里把 `yizhi/` 组装到 `_site/`（顺手排除过程文件）→ `upload-pages-artifact` → `deploy-pages` | 多一个 `.github/workflows/pages.yml`；**换来发布目录可自由指定 + 可编程排除 + 不污染根目录** |
| B · 复制到根目录 | 把 `yizhi/` 里所有内容复制到仓库根目录 | 根目录被站点文件塞满，`PRD.md`／`checkin/` 混在一起，**不推荐** |

**为什么不用「Deploy from a branch」**：那条路会跑 Jekyll，中文文件名与 `{{` 之类字符会被当模板处理，而且**没法排除 `*.md`**（底稿和台账会直接挂到公网）。artifact 直传是产物原样上线，没有任何加工。

**流水线里的三道自检**（`.github/workflows/pages.yml`）：
1. `rsync` 组装 `_site/`，排除 `*.md` / `预览/` / `assets-private/` / `_*`（临时探针脚本）；
2. 断言 `_site/index.html` 存在；
3. **反查 `_site/` 里还有没有漏网的过程文件，有就让构建红掉** —— 发布纪律靠 CI 兜，不靠人记。

**人机分工（M7 执行时）**：workflow 文件我写；宝哥在 GitHub 网页上点 `Settings → Pages → Source` 选 **GitHub Actions**，然后在 Actions 页看一次运行是否绿勾。这属于必须人工点的步骤，我不代做。
...
**发布内容排除清单**：`预览/`、`*.md`、`assets-private/`、以 `_` 开头的临时脚本（探针 / 校验）。
（`assets/SOURCES.md` 是来源台账，属维护资料，被 `*.md` 一并排除。）

实测定量（M7 本地按同规则模拟）：`yizhi/` 共 53 个文件 → **发布 39 个、挡下 14 个**。挡下的 14 个 = 6 个临时脚本 + 3 份底稿/口径 `.md` + `assets/SOURCES.md` + `assets-private/SOURCES.md` + `预览/` 下 2 个样张 + `预览/卡片样式v1.html`。**运行时一个都不缺。**
...
>
> ⚠️ **M7 已到（09-22 末）：这条现在必须拍板了**，摆进 §14 待决清单 —— 见第 6 项。
...
| 3 | 发布用 GitHub Actions（而非复制到根目录） | ✅ **M7 已落**：`.github/workflows/pages.yml`（§11.1/11.2）。剩人工步骤：GitHub 网页 `Settings → Pages → Source = GitHub Actions` |
| 4 | 本地开发是否需要本地服务兜底 | ✅ **已实测（Day 7）**：`file://` 下模块脚本与 `fetch` 双双被拦，双击只剩空骨架 → 本地**必须**起服务。宝哥 09-22 定：双击这条**暂缓到 M7 前再评估**（详见 §11.5） |
| 5 | **`file://` 双击到底要不要救**（M7 已到期，必须拍板） | ⬜ 三选一：**A** 不救，README 写明「必须起本地服务」（代价：零；PRD §8.6 那条验收要改口径）／**B** 放弃 `type="module"` + 数据内联，换双击可用（代价：打破「JSON 是唯一真源」§12.2，且数据一改要重新内联）／**C** 加 `--allow-file-access-from-files` 这类浏览器参数（代价：要求用户改浏览器设置，等于没有）。**我的建议：A** —— 站点是要上线的，本地起一行命令的成本远低于把数据真源拆成两份；把 PRD §8.6 改成「线上打开」即可 |
| 6 | **搜「龙」零结果**（全站没有任何条目叫龙，只在几处原文里顺带出现） | ⬜ 索引只建 `名/别名/关键词`（§5.2），**不含 `原文`** —— 这是设计，不是 bug。但「龙」是中国超自然里最热的一个词，搜出 0 条会被当成站坏了。三选一：**A** 不动（零结果页有出口，不是死胡同）／**B** 给相关条目补 `关键词`（如 大禹治水→「应龙」、女娲补天→「龙」），**不改名单、不加条目**／**C** 把 `原文` 也纳入索引（代价：原文是长引文，噪音大，搜「之」「其」会命中一大片）。**我的建议：B** —— 成本最低、不动已拍板的 12 条名单，且能兜住最常见的几个词 |
| 7 | 检索结果卡**不显示命中理由** | ⬜ 例如「筑基」出现在搜「丹」的结果里，卡面看不出为什么（实际靠关键词「筑基丹」命中）。要修就得让 `result.js` 拿到命中字段（`query` 目前只返回条目，不返回命中来源），**契约要加一个返回值**。**我的建议：先记着** —— 结果条数少、命中一般不意外，等真有人被绕晕再做 |
| 8 | 仓库根 `index.html` 占位页（Day 2「Vibe Coding 打卡学习」）不进发布 | ⬜ 站点根 = 异兽志，占位页线上打不开。要不要让 `/` 是占位页、`/yizhi/` 是站（改一行 `path`）？**我的建议：不改成** —— 占位页没有信息量，让产品直接站在域名根更好 |
| 9 | 小旋钮（沿用不改架构） | ⬜ 书签上的文字 / 云纹与印章的浓度 / 零结果页「清空搜索」这个出口的位置 |
| 10 | 12 张配图中 **11 张待宝哥过目** | ⬜ 出图已入库（`assets/`），但视觉验收要人眼看；我这边只保证了「纸底/墨量/朱砂占比」的量化门槛 |
