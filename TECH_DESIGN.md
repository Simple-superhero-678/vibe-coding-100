...
**素材从知识库人工录入 `data/*.json`；页面打开时浏览器把 JSON 读走、渲染成五个视图；收藏只写回浏览器自己的 `localStorage`；整站由 GitHub Pages 发文件。没有后端、没有数据库、没有密钥、没有构建工具。**
...
| 图形 | 自绘 SVG 剪影 | — | 版权可控、体积小、颜色可跟随三色约束 |
...
| 网络图片 / 图库 | 版权不可控且破坏三色约束（PRD 第 7 节「搬运网络图片 = 会偏」） |
...
| `svg` | string | ⛔ | 对应 `svg/{值}.svg` | 渲染兜底占位框，不显示破图标 |
...
renderDetail(条目)                   // V3 详情面板
renderFavList(容器, 条目[])          // V4 收藏列表
```
...
- **源头**：ima 库 `3-Resources/传统文化` 下的 6 篇摘要（上古神话 / 山海经异兽 / 妖怪文化 / 道教神仙体系 / 网文修仙体系 / 传统文化素材地图）。
- **这一步不能自动化**。条目里的「写小说怎么用」「异说」需要人的判断，脚本产不出来（PRD 9.3：先深后广）。
- **JSON 是唯一真源**。改内容只改 JSON，改完提交，不用管别的。
...
> 源文件：`TECH_DESIGN-数据流图.svg`（矢量图，可任意放大截图）
...
flowchart TB
    subgraph L1["① 内容流 · 数据从哪来 —— 人做，一批一次"]
        direction LR
        A1["ima 知识库<br/>传统文化 6 篇摘要"] --> A2["人工提炼录入<br/>读素材，写条目"]
        A2 --> A3["data/*.json<br/>唯一真源"]
        A3 --> A4["git 提交推送<br/>每次一版记录"]
        A4 --> A5["GitHub Pages<br/>只发文件，不跑逻辑"]
    end

    subgraph L2["② 用户流 · 数据到哪去 —— 机器做，全程不出浏览器"]
        direction LR
        B1["浏览器 fetch<br/>读 data/*.json"] --> B2["校验 + 建索引<br/>缺必填的不进列表"]
...
| 来源 | 提供什么 | 谁负责 |
|---|---|---|
| **ima 知识库**《传统文化》6 篇摘要 | 原文、出处、异说的原始素材 | 已有资产，Day 3 起就在用 |
| **人工提炼** | 把素材写成合规条目（补上「写小说怎么用」这类需要人判断的内容） | **必须人做**，脚本产不出来 |
...
**一句话版本**：
> 素材从知识库来，经人手写成 JSON，推上 GitHub 后由 Pages 发给浏览器，浏览器把它渲染成画面；用户的收藏往回写进浏览器自己的存储里，导出时脱离网站变成一份纯文本。
...
| 10 | SVG 剪影文件缺失 | `<img>`/`fetch` 的 error | 兜底占位框（纸色底 + 细边框） | 一个安静的空框 | 不显示破图标、不显示 alt 文字外露 |
...
**发布内容排除清单**：`预览/`、`*.md`、`.gitkeep` 之外的临时文件。
...
⚠️ 但 `fetch` 读本地 JSON 在部分浏览器会被 CORS 拦（`file://` 协议）。所以本地开发时**推荐起一个极简本地服务**（如 `python -m http.server`），双击打不开 JSON 时用它兜底——这是**开发时的便利**，不是发布形态；发布后走 `https://` 一切正常。
...
| 4 | 本地开发是否需要本地服务兜底 | 待 Day 7 实际验证 `file://` 下能否读到 JSON |
